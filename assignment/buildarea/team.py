        ...
        for z in range(len(house_grid[0])):
            if house_grid[x][z] == 1:
                ...
                build_funcs.remove_trees(
                    ED, xstart, ystart, zstart, size_struct, size_struct, size_struct
                )
                ...
                with ED.pushTransform(
                    ...
                    build_funcs.build_exterior_one_house_part(
                        ED, size_struct, size_struct, size_struct, 0, 1
                    )
...
def build_boxes(mymap: MapHolder, it):
    ...
    while helper.is_tree(
        ED, STARTX + best_loc_x, best_loc_y, STARTZ + best_loc_z
    ) or helper.is_air(ED, STARTX + best_loc_x, best_loc_y, STARTZ + best_loc_z):
        best_loc_y -= 1
