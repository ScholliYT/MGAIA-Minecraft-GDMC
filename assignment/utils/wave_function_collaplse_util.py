...
def collapse_unbuildable_to_air(wfc: WaveFunctionCollapse, buildable: List[List[bool]]):
    ...
    """
    if wfc.state_space_size[0] - 2 != len(buildable) or wfc.state_space_size[2] -2 != len(buildable[0]):
        raise ValueError("WFC state space size should have a padding of 1 around the buildable area")

    for x in range(len(buildable)):
        for z in range(len(buildable[x])):
            if not buildable[x][z]:
                wfc.collapse_cell_to_state([x+1,0,z+1], StructureRotation(empty_space_air, 0))
