...
def terraform_distance(
    heights: np.ndarray, y: int, origin: Tuple[int, int], size: Tuple[int, int]
) -> int:
    ...
    heights_slice = heights[origin[0] : origin[0] + size[0], origin[1] : origin[1] + size[1]]
...
def score_all_possible_buildregions(
    heights: np.ndarray,
    square_sidelenght=11,
    min_adjecent_squares=1,
    ...
    sidelengths = range(
        square_sidelenght * min_adjecent_squares + 2 * buffer,
        square_sidelenght * max_adjecent_squares + 1 + 2 * buffer,
        square_sidelenght,
    )
