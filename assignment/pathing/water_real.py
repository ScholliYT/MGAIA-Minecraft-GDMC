...
def get_neighbour_paths(paths, map_avoid, slope):
    path_array = np.zeros_like(map_avoid)
    ...
    path_array -= slope * SLOPE_AVOID_MULT

    return path_array


def make_paths(slope: np.ndarray, building_map: np.ndarray) -> np.ndarray:
    # load maps
    # slope = np.loadtxt('slope.txt', dtype=float)
    # map = np.loadtxt('map.txt', dtype=float)

    map_avoid = gaussian_filter(building_map, sigma=3, mode="constant")

    doors = []
    for row_nr in range(len(building_map)):
        for col_nr in range(len(building_map)):
            if building_map[row_nr][col_nr] == 2:
    ...
    # copy the map for visualization
    mapcopy = building_map.copy()
    final_paths = np.zeros_like(building_map, dtype=int)
    for p in paths:
        p = fix_path_final(p)
        for block in p:
            if building_map[block[0], block[1]] == 0:
                mapcopy[block[0], block[1]] = 3
                final_paths[block[0], block[1]] = 1

    time.sleep(0.1)

    print("Finished generation of paths. Please close the window after inspecting the paths.")
    plt.imshow(np.flip(mapcopy, axis=0), cmap="hot", interpolation="nearest")
    plt.colorbar()
    plt.title("Paths connecting buildings")
    plt.show()
