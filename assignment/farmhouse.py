...
def build_farmhouse_wall(bottom_left: ivec3 = ivec3(0, 0, 0)):
    ...
    geo.placeCuboidHollow(
        ED, bottom_left + ivec3(0, 0, 1), bottom_left + ivec3(0, 2, 5), Block("cobblestone")
    )
    geo.placeLine(
        ED, bottom_left + ivec3(0, 1, 2), bottom_left + ivec3(0, 1, 4), Block("glass_pane")
    )
    geo.placeLine(ED, bottom_left, bottom_left + (0, 2, 0), Block("oak_log"))

    # add floor
    geo.placeCuboid(
        ED, bottom_left + ivec3(0, -1, 0), bottom_left + ivec3(5, -1, 5), Block("oak_planks")
    )
...
def build_farmhouse_wall_double_pillar(bottom_left: ivec3 = ivec3(0, 0, 0)):
    ...
    # on the bottom left block 1x3x6
    geo.placeCuboidHollow(
        ED, bottom_left + ivec3(0, 0, 1), bottom_left + ivec3(0, 2, 4), Block("cobblestone")
    )
    geo.placeLine(
        ED, bottom_left + ivec3(0, 1, 2), bottom_left + ivec3(0, 1, 3), Block("glass_pane")
    )
    geo.placeLine(ED, bottom_left, bottom_left + (0, 2, 0), Block("oak_log"))
...
def build_farmhouse_corner(bottom_left: ivec3 = ivec3(0, 0, 0)):
    ...
    geo.placeLine(ED, bottom_left, bottom_left + (0, 2, 0), Block("oak_log"))
    geo.placeCuboidHollow(
        ED, bottom_left + ivec3(1, 0, 0), bottom_left + ivec3(5, 2, 0), Block("cobblestone")
    )
    geo.placeLine(
        ED, bottom_left + ivec3(2, 1, 0), bottom_left + ivec3(4, 1, 0), Block("glass_pane")
    )
...
def build_farmhouse_wall_door(bottom_left: ivec3 = ivec3(0, 0, 0)):
    ...
    geo.placeCuboid(
        ED, bottom_left + ivec3(0, 0, 0), bottom_left + ivec3(0, 2, 1), Block("oak_log")
    )
    ...
    geo.placeCuboid(
        ED, bottom_left + ivec3(0, 0, 5), bottom_left + ivec3(0, 2, 5), Block("oak_log")
    )
...
def main():
    try:
        ...
        prefabs = [
            ...
            build_farmhouse_wall_double_pillar,
            build_farmhouse_corner,
            build_farmhouse_wall_door,
        ]
        with ED.pushTransform(Transform(translation=ivec3(-80, 0, 120))):
            # clear area
            geo.placeCuboid(
                ED, ivec3(0, -1, 0), ivec3(10, 2, 0) + len(prefabs) * ivec3(0, 0, 10), Block("air")
            )
            ED.flushBuffer()
