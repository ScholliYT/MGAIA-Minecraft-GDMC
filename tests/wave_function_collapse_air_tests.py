...
class WaveFunctionCollaplse2x1x2_Air_Test(unittest.TestCase):
    ...
    def test_collapses_with_entrance_top_right(self):
        ...
        #   air         - entrance(0)
        #   entrance(2) - air
        bottom_left_subset = all_air_structures.union(
            set([StructureRotation(brickhouse_entrance, 2)])
        )
        self.assertTrue(
            bottom_left_subset.issubset(self.wfc.state_space[0][0][0]),
            msg=f"{bottom_left_subset} should be a subset of {self.wfc.state_space[0][0][0]}",
        )
        ...
        self.assertEqual(
            set([StructureRotation(brickhouse_entrance, 0)]), self.wfc.state_space[1][0][1]
        )
