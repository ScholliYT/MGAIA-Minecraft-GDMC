...
class TestOptimizer(unittest.TestCase):
    ...
    def test_optimize_2x2(self):
        heightmap = np.array([[5, 4], [7, 5]])
        result = list(
            score_all_possible_buildregions(heightmap, square_sidelenght=1, max_adjecent_squares=2)
        )
    ...
    def test_optimize_5x5(self):
        heightmap = np.array(
            [[5, 4, 5, 5, 5], [6, 5, 5, 4, 5], [7, 8, 5, 5, 5], [4, 4, 4, 4, 4], [3, 3, 3, 3, 3]]
        )

        result = list(
            score_all_possible_buildregions(heightmap, square_sidelenght=1, max_adjecent_squares=3)
        )
    ...
    def test_optimize_25x25(self):
        ...
        result = list(
            score_all_possible_buildregions(
                heightmap, square_sidelenght=11, min_adjecent_squares=1, max_adjecent_squares=4
            )
        )
